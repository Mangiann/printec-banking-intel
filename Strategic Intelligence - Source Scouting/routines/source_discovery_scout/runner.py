...
def legacy_tolerant_ssl_context():
    """An SSLContext that still talks to the old servers a browser talks to, cached module-wide.

    Python 3.14 / OpenSSL 3.x ship a default cipher list with FORWARD SECRECY ONLY -- no static-RSA
    key exchange. Several real sources in the footprint are hosted on servers whose single offered
    cipher is `AES128-GCM-SHA256` (TLS_RSA_WITH_AES_128_GCM_SHA256, TLS1.2, no FS): apek.cz and
    ekap.kik.gov.tr are the measured ones. There is then ZERO cipher overlap, the server aborts with
    `SSLV3_ALERT_HANDSHAKE_FAILURE`, and the verifier records a LIVE source as `failed` -- a pure
    false negative, since every browser still loads those pages.

    `@SECLEVEL=1` re-admits those legacy suites. CERTIFICATE VERIFICATION IS DELIBERATELY LEFT ON:
    the expired certs and hostname mismatches in the failure set (cezap.me, data.isrs.cz) are REAL
    defects a browser also warns about, and must keep failing rather than be waved through here.
    """
    global _TLS_CTX
    if _TLS_CTX is None:
        ...
        except ssl.SSLError:      # a build with no such level -- keep the stock context
            pass
        _TLS_CTX = ctx
    return _TLS_CTX
...
class HttpUrlVerifier:
    ...
    mock) and NOT used by the default offline config. Best-effort HEAD via stdlib urllib, falling
    back to GET when the server refuses HEAD."""
    ...
    def _fetch(self, url, method):
        ...
        req = urllib.request.Request(url, method=method,
                                     headers={"User-Agent": "printec-scout-runner/0.1"})
        with urllib.request.urlopen(req, timeout=self.timeout,
                                    context=legacy_tolerant_ssl_context()) as resp:
            if method == "GET":
                resp.read(2048)          # touch the body so it is a real GET, then drop it
    ...
    def check(self, url):
        import urllib.error
        try:
            return self._fetch(url, "HEAD")
        except urllib.error.HTTPError as e:
            # 405/501 = "HEAD not allowed here", not "dead". Retry the way a browser would ask.
            if e.code in (405, 501):
                try:
                    return self._fetch(url, "GET")
                except urllib.error.HTTPError as e2:
                    return {"reachable": 200 <= e2.code < 400, "http_status": e2.code,
                            "final_url": url, "error": f"HTTP {e2.code}"}
                except Exception as e2:  # noqa: BLE001
                    return {"reachable": False, "http_status": None, "final_url": None,
                            "error": str(e2)}
